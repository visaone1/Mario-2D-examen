...
void RetrieveGameState() {
    // 1. Buscamos el gestor de la forma en que lo hace este proyecto
    GameStateManager t_GameStateManager = FindObjectOfType<GameStateManager>();

    // 2. Si no lo encuentra (porque iniciaste directo en el nivel), creamos los valores temporales
    if (t_GameStateManager == null) {
        Debug.LogWarning("Iniciando desde el nivel. Creando estado temporal.");
        lives = 3;
        timeLeft = 400;
        return; 
    }
    
    // 3. Si SÍ lo encuentra (porque vienes del Menú), carga los datos normales
